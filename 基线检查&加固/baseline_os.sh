...
print_info "==> 推荐的文件权限:"
print_info "/var/log/messages、/var/log/secure、/var/log/maillog、/var/log/cron <= 600"
print_info "/var/log/dmesg <= 644 、/var/log/wtmp <= 664"
...
if [ -z "$tmp" ]; then
  ...
  print_info "==> 建议local_umask和anon_umask为022"
...
print_info "==> 建议文件权限：/etc/passwd <= 644、/etc/shadow <= 400、/etc/group <= 644"
print_info "passwd==>""$passwd"
print_info "shadow==>""$shadow"
...
if [ -n "$tmp" ]; then
  ...
  print_info "/etc/vsftpd/ftpusers 配置文件中是否有root"
...
echo "auth  required pam_wheel.so group=wheel"
